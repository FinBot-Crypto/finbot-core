...
async def get_global_settings():
    ...
    try:
        conn = get_db_conn()
        cur = conn.cursor()
        settings = _fetch_global_settings(cur)
        cur.close()
        return settings
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_global_settings(payload: dict):
    conn = None
    try:
        ...
        conn.commit()
        cur.close()
        return {"status": "success", "message": "Configurações globais salvas."}
...
@app.get("/api/settings/block/{block_id}")
async def get_block_settings(block_id: str):
    conn = None
    try:
        conn = get_db_conn()
        cur = conn.cursor()
        settings = _fetch_block_settings(cur, block_id)
        cur.close()
        return settings
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@app.post("/api/settings/block/{block_id}")
async def update_block_settings(block_id: str, payload: dict):
    conn = None
    try:
        updates = {k: v for k, v in payload.items() if not k.startswith("_")}
        for k, v in updates.items():
        ...
        for k, v in updates.items():
            cur.execute(
                ...
                DO UPDATE SET value = EXCLUDED.value, updated_at = CURRENT_TIMESTAMP
                """,
                (block_id, k, json.dumps(v)),
            )
        conn.commit()
        cur.close()
        return {"status": "success", "message": f"Configurações de {block_id} salvas.", "block": block_id}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@app.get("/api/mare/signals")
async def get_mare_signals(limit: int = Query(100, ge=1, le=500)):
    """Observability endpoint for Maré; it never exposes secrets or enables trading."""
    conn = None
    try:
        conn = get_db_conn()
        cur = conn.cursor()
        cur.execute(
            f"""
            SELECT id, symbol, direction, score, accepted, reason,
                   tide_state, wave_state, ripple_state, price, atr,
                   block_enabled, live_orders_enabled,
                   evaluated_at {TZ_EXPR} AS evaluated_at
            FROM mare_signals
        ...
        return [
            {
                ...
                "score": float(row[3]), "accepted": row[4], "reason": row[5],
                "tide": row[6], "wave": row[7], "ripple": row[8],
                "price": float(row[9]), "atr": float(row[10]),
                "block_enabled": row[11], "live_orders_enabled": row[12],
                "evaluated_at": row[13].isoformat() if row[13] else None,
            }
            for row in rows
        ]
    except Exception as exc:
        # The Core can boot before the optional Maré migration is installed.
        if "mare_signals" in str(exc):
            return []
