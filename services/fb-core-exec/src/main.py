...
class CoreExecService:
    ...
    async def process_order(self, msg):
        try:
            ...
            for item in orders:
                ...
                if opened:
                    await self.js.publish("trade.opened.v1", json.dumps(opened).encode())
            await msg.ack()
        except Exception as exc:
    ...
    async def run(self):
        ...
        for subject, durable in (("trade.order.v1", "CORE_EXEC_V1_WORKER"), ("trade.order", "CORE_EXEC_LEGACY_WORKER")):
        ...
        for subject, durable in (("trade.close.v1", "CORE_EXEC_CLOSE_V1_WORKER"), ("trade.close", "CORE_EXEC_CLOSE_LEGACY_WORKER")):
            await self.js.subscribe(
                subject,
                durable=durable,
