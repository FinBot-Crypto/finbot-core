...
class CoreMonitorService:
    ...
    async def close_position(self, pos: dict, reason_data: dict):
        ...
        if not DRY_RUN and reason_data["reason"] != "EXCHANGE_CLOSED":
            ...
            await self.js.publish("trade.close.v1", payload)
        ...
        await self.js.publish("trade.closed.v1", json.dumps(closed).encode())
    ...
    async def run(self):
        await self.connect_nats()
        await self.load_positions_from_kv()
        for subject, durable in (("trade.opened.v1", "CORE_MONITOR_OPENED_V1"), ("trade.opened", "CORE_MONITOR_OPENED_LEGACY")):
